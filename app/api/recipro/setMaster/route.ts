...
export async function POST(req: Request) {
  ...
  const { customerID, storeID, setData } = body ?? {};

  if (!customerID || !storeID || !Array.isArray(setData)) {
    return NextResponse.json(
      { error: "Invalid request body: customerID, storeID, setData are required" },
      { status: 400 }
    );
  }
  ...
  // 7. 外部 Recipro setAdminMaster を admin として呼ぶ
