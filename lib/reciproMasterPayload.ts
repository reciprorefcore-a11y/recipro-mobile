...
export function buildReciproMasterPayload(params: {
  ...
  const setData = activeIngredients.map((item) => {
    const inputQuantity = getInputQuantity(item);
    ...
    return {
      ...
      "［取引先名］": item.supplier ?? "",
      "［単価変更日］": "",
      "［自社管理入数］": toPayloadText(item.inputQuantity),
      "［自社管理入数単位］": item.inputQuantityUnit ?? "",
      "［発注単価］": "",
      "［発注単位］": "",
      "［マイカタログ単価］": "",
      "［マイカタログ旧単価］": "",
      ...
      "［入数］": `${toPayloadText(inputQuantity)}\r`,
    };
  });
...
function getInputQuantity(item: Ingredient) {
  return firstPresent(
    item.irisu,
    item.inputQuantity,
    item.packQuantity,
...
function toPayloadText(value: string | number | undefined): string {
